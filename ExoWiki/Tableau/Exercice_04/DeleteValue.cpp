...
bool supprimerParValeur(std::vector<int>& table, int valeur) {
...
int main(){
    ...
    printTable(table);
    std::cout << "Entrez la valeur à supprimer: ";
    std::cin >> value;
    if (supprimerParValeur(table, value)) {
        std::cout << VERT << "Valeur " << value << " supprimée du tableau." << RESET << std::endl;
    } else {
        std::cout << ROUGE << "Valeur " << value << " non trouvée dans le tableau." << RESET << std::endl;
    }
    std::cout << "Tableau après suppression: " << std::endl;
    printTable(table);
